...
function applyGradeBadge(badge, letterEl, percentEl, grade) {
  badge.className = "grade-badge " + getGradeClass(grade.percent);
...
function renderClassCard(cls) {
  ...
  applyGradeBadge(badge, letterEl, percentEl, grade);
  ...
  whatifToggle.addEventListener("click", () => {
    const isOpen = !whatifPanel.hidden;
    whatifPanel.hidden = isOpen;
    whatifToggle.classList.toggle("is-open", !isOpen);
  });
  ...
  node.querySelector(".whatif-run-btn").addEventListener("click", () => {
    ...
    resultEl.textContent =
      `Scoring ${score}/${max} → new grade: ${hypo.percent.toFixed(1)}% (${hypo.letter})` +
      ` — from ${currentTxt}${diffTxt}`;
  });
